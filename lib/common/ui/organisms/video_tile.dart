import 'package:golive/common/ui/organisms/change_track_options.dart';
...
class VideoTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (index > 0 && filteredList[0].source == "SCREEN") {
      ...
      index = (a - 1) * ((orientation == Orientation.portrait) ? 4 : 2) + (b + 1);
    ...
    return VisibilityDetector(
      onVisibilityChanged: (VisibilityInfo info) {
        ...
        if (visiblePercentage <= 40) {
          ...
          map[trackId] = filteredList[index].isMute ? HMSTrackUpdate.trackMuted : HMSTrackUpdate.trackUnMuted;
          print(map[trackId]);
        }
        debugPrint('Widget ${info.key} is $visiblePercentage% visible and index is $index');
      },
      key: Key(filteredList[index].trackId),
      child: InkWell(
        onLongPress: () {
          if (!filteredList[index].peer!.isLocal && filteredList[index].source != "SCREEN")
            showDialog(
                context: context,
                builder: (_) => Column(
                      children: [
                        ChangeTrackOptionDialog(
                            isAudioMuted: _meetingStore.audioTrackStatus[filteredList[index].trackId] ==
                                HMSTrackUpdate.trackMuted,
                            isVideoMuted: map[filteredList[index].trackId] == HMSTrackUpdate.trackMuted,
                            ...
                            changeTrack: (mute, isVideoTrack) {
                              ...
                              if (filteredList[index].source != "SCREEN")
                                _meetingStore.changeTrackRequest(
                                    filteredList[index].peer?.peerId ?? "", mute, isVideoTrack);
                            },
                            removePeer: () {
                              Navigator.pop(context);
          ...
          return ShouldRebuild<PeerItemOrganism>(
            shouldRebuild: (oldWidget, newWidget) {
              return oldWidget.track != newWidget.track ||
                  ...
                  oldWidget.track.isHighestAudio != newWidget.track.isHighestAudio;
            },
            child: PeerItemOrganism(
                ...
                isVideoMuted: filteredList[index].peer!.isLocal
                    ...
                    : (map[filteredList[index].trackId]) == HMSTrackUpdate.trackMuted),
          );
        }),
      ),
    );
